...
- RoundTable is the central record of all sessions, decisions, and output to Commander ท่านผู้บัญชาการ
...
- Log entries must include:
  ...
  - Discussion content
  - Decisions made
  - Output to Commander ท่านผู้บัญชาการ from the prompt execution
...
**Per-Interaction Entry (required after EVERY exchange with Commander ท่านผู้บัญชาการ):**
```markdown
...
### Prompt from Commander ท่านผู้บัญชาการ
...
### AS (Overseer — Design & Verification Scholar)   ← only if topic is UX/QA/flow
"[Design or verification observation]"
...
### Output Delivered
**Reason:** [One sentence: why this output was produced — what the Commander asked for]

[Full verbatim content: commands, tables, code blocks, configs, step-by-step procedures, key conclusions]
```

**Output Logging Rule — MANDATORY from Session 25 onward (10-03-2026):**

Every session entry that produces a response to Commander ท่านผู้บัญชาการ MUST include an `### Output Delivered` block containing the full, verbatim content of what was presented — commands, tables, configs, code blocks, recommendations, and key conclusions.
...
- If the Commander's prompt was answered with a single sentence, that sentence is the Output Delivered
...
- The `**Reason:**` line is mandatory — it links the output to the Commander's intent so the record is self-explanatory without reading the prompt
...
- Shortcuts that compromise quality, security, or maintainability must be explicitly flagged as Technical Debt and require Commander ท่านผู้บัญชาการ sign-off
- "Good enough" is only acceptable when the Commander ท่านผู้บัญชาการ has explicitly approved a scoped trade-off
...
- Any team member **may hold and express a conflicting view** with the Commander ท่านผู้บัญชาการ or any other member when they believe a different approach leads to a better project outcome
...
- The Commander ท่านผู้บัญชาการ has final authority on all decisions — but the team's job is to ensure that authority is exercised with full information
- After the Commander ท่านผู้บัญชาการ's decision is final, the team executes without further debate

**Team Chat (All other teams):**
- Teams **Monolith**, **Syndicate**, and **Arcade** log their discussions in `.claude/Team Chat`
...
### Prompt from Commander ท่านผู้บัญชาการ (Commander ท่านผู้บัญชาการ)   ← omit if AM-initiated
> "[direct quote from Commander ท่านผู้บัญชาการ]"
...
### AS (Overseer — Design & Verification Scholar)   ← speaks on UX/flow/schema topics AND QA/test/acceptance topics
"[User experience, flow design, structural observations, test coverage, acceptance criteria, output integrity]"
...
- AS never references code internals, schemas, or architecture — UX, flow, QA, and acceptance only
- All 3 members must contribute in sessions involving major decisions (All-Voices Rule)
...
|------|--------|------|----------|------------|
| DD-MM-YYYY | 1 | `DD-MM-YYYY_RoundTable.md` | 1–17 | L3 scan, BOS.Kit brainstorm |
| DD-MM-YYYY | 2 | `DD-MM-YYYY_RoundTable_Vol2.md` | 18–34 | CLAUDE.md restructure, RoundTable rotation |
```
...
*Extracted from CLAUDE.md §1 + RoundTable Session Standard + new Rotation Policy — 11-03-2026*
